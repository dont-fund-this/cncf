...
def with_import_real_flags() {
    return {
        "sql.import.real.flags",
        "sql",
        [](Address address, Payload, Options) -> bool { return fit_import_real_flags(address); },
...
using nlohmann::json;
            ...
            for (const auto& f : build_import_frames(target, table, file, {}, flags))
                send_to(options, f, {});
            return 1;
        },
    };
