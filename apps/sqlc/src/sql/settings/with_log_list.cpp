...
def with_log_list() {
    return {
        "sql.log-list",
        "sql",
        [](Address address, Payload, Options) -> bool { return fit_log_list(address); },
...
using nlohmann::json;
            ...
            const json frames = exec(db, "SELECT LogTime, LogCode, LogText FROM LogData ORDER BY rowid DESC LIMIT " + std::to_string(limit));
            db_close(db);
