...
fn extract_namespace<'a>(addr: &'a [u8]) -> Option<&'a [u8]> {
    let prefix = b"/api/v1/namespaces/";
    let rel_prefix = b"namespaces/";

    let rest = if addr.starts_with(prefix) {
        &addr[prefix.len()..]
    } else if addr.starts_with(rel_prefix) {
        &addr[rel_prefix.len()..]
    } else {
        return None;
    };
    ...
    if after == b"/serviceaccounts" && !ns.is_empty() {
        Some(ns)
    } else {
        None
    }
...
pub static ServiceAccountPost: Def = Def {
    sid: b"/api/v1/namespaces/serviceaccounts\0".as_ptr() as *const i8,
    tag: b"/api/v1/namespaces/serviceaccounts\0".as_ptr() as *const i8,
    fit: |address: Address, _p: Payload, options: Options| -> bool {
        if address.is_null() {
            return false;
        }
        let addr = unsafe { CStr::from_ptr(address).to_bytes() };
        extract_namespace(addr).is_some() && has_verb(options, b"POST")
    },
    fun: |address: Address, payload: Payload, options: Options| -> i32 {
        ...
        let addr = unsafe { CStr::from_ptr(address).to_bytes() };
        let ns = match extract_namespace(addr) {
            Some(n) => n,
            None => return -1,
        };
        ...
        let mut buf = [0u8; 512];
        let p1 = b"{\"apiVersion\":\"v1\",\"kind\":\"ServiceAccount\",\"metadata\":{\"name\":\"";
        let p2 = b"\",\"namespace\":\"";
        let p3 = b"\",\"uid\":\"1\",\"resourceVersion\":\"1\"}}\0";

        let mut idx = 0;
        buf[idx..idx + p1.len()].copy_from_slice(p1);
        idx += p1.len();
        buf[idx..idx + sa.len()].copy_from_slice(sa);
        idx += sa.len();
