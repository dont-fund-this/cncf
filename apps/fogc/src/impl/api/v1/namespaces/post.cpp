...
extern const Def NamespacePost = {
    ...
    [](Address address, Payload, Options options) -> bool {
        ...
        return (addr == "/api/v1/namespaces" || addr == "namespaces") && has_verb(options, "POST");
    },
    [](Address address, Payload payload, Options options) -> int {
        if (!NamespacePost.fit(address, payload, options)) return -1;
        std::string ns = extract_name(payload);
